...
test("without a judge the heuristic order is returned untouched", async () => {
...
test("a single commit is never sent to the model", async () => {
...
test("model scores reorder the batch and the heuristic order breaks ties", async () => {
  ...
  const result = await rerankByWit(commits, judge);
  // b wins outright, a and c tie on 4 and keep their incoming order, d sinks.
...
test("commits past the request cap keep their place and none are dropped", async () => {
  ...
  const judge: WitJudge = async (subjects) => {
    ...
    // Reverse the judged slice so the reordering is unmistakable.
